...
/*
  URL : /communities/:communityId/role/:role/beInvitedUsers
*/
router.route('/')
.get((req, res) => {
  if (permission.isAllowed(req.user.permission, `InviteMembers:${req.role}:create`))
    invitation.getBeInvitedUsers(req.role, req.communityId).then(result => { return http.success(req, res, result) })
  else

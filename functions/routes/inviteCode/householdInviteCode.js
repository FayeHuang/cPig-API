...
/*
  URL : /communities/:communityId/households/:householdId/role/:role/inviteCode
*/
router.route('/')
.get((req, res) => {
  if (permission.isAllowed(req.user.permission, `MemberInviteCodes:${req.role}:own:read`))
    inviteCode.getOne(req.role, req.user.uid, req.communityId, req.householdId).then(result => { return http.success(req, res, result) });
  else
    return http.permissionDenied(req, res);
})
.post((req, res) => {
  if (permission.isAllowed(req.user.permission, `MemberInviteCodes:${req.role}:create`))
    inviteCode.create(req.role, req.user.uid, req.communityId, req.householdId).then(result => { return http.success(req, res, result) });
  else
    return http.permissionDenied(req, res);
})
.put((req, res) => {
...
.delete((req, res) => {
  if (permission.isAllowed(req.user.permission, `MemberInviteCodes:${req.role}:own:delete`))
    inviteCode.remove(req.role, req.user.uid, req.communityId, req.householdId).then(() => { return http.success(req, res) });
  else
